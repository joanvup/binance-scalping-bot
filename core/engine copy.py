...
class TradingEngine:
    ...
    async def _update_funding_loop(self):
        ...
        while True:
            if self.mode != "DRY_RUN" and state.is_running:
                try:
                    # 1. Sincronizar el reloj local cada 10 minutos (10 loops de 60 segundos)
                    if loop_count % 10 == 0:
    ...
    async def start(self):
        ...
        # Iniciar el monitor de financiación en segundo plano
        asyncio.create_task(self._update_funding_loop())

        bsm = BinanceSocketManager(binance_manager.client)
        async with bsm.kline_socket(symbol=self.symbol, interval=self.interval) as stream:
            logger.info(f"Escuchando WebSockets de Binance para {self.symbol}...")
            ...
            while True:
                ...
                if 'k' in msg:
                    ...
                    # --- ACTUALIZAR VELA Y VALORES EN VIVO ---
                    ...
                    if is_kline_closed:
                        # Guardar la vela cerrada en el historial del Dashboard
